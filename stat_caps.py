...
#############################################
#### Create atoms around a core 2 cluster   #
#############################################
...
# start investigation of different angles
    n_atoms = len(unique)
    angles  = []

    for i in range(0,n_atoms):
        veca  = unique[i]
        anorm = np.linalg.norm(veca)
    ...
    if nr_checked == 0:
        ...
        for compare in range(0,nr_checked):
            ...
            if dnorm < 1.0E-8 :
                all_numbers[compare] = all_numbers[compare] + 1
                found = 1
        if found != 1:
            all_angles = np.vstack((all_angles,npangles))
            all_numbers.append(1)
            all_combis.append(caps)
        found = 0
    nr_checked = len(all_numbers)
#    print nr_checked
    for h in range(0,len(unique)):
